...
- `Real UDP Lab`
  실제 UDP 서버 소켓과 실제 UDP 소켓을 실행하고, datagram 송수신/close/error를 로그 파일과 라이브 UI에 남긴다.

즉, 이 프로젝트는 "TCP/UDP 개념 학습용 화면", "실제 TCP 실험실", "실제 UDP 실험실"을 함께 제공한다.
...
상단 탭의 `실제 UDP 실험실` 페이지에서는 실제 Node UDP 서버 소켓을 바인드하고, 실제 Node UDP 소켓을 만들어 datagram을 주고받을 수 있다.

이 페이지에서는 아래를 실제로 확인할 수 있다.

- UDP 서버 소켓이 실제로 bind 되는지
- 어떤 local address, port에서 datagram을 보내고 받는지
- 최근 상대 주소와 포트가 어떻게 바뀌는지
- 각 소켓이 몇 바이트, 몇 개의 datagram을 썼고 읽었는지
- UDP 로그 파일에 어떤 기록이 남는지
- `nc -u`, `lsof`, `tcpdump`, Wireshark 같은 외부 도구로도 같은 datagram을 볼 수 있는지
...
- 실제 `dgram.createSocket("udp4")` 서버 소켓 bind
- 실제 UDP 소켓 bind
- 실제 datagram 전송과 반대편의 `message`
- 최근 상대 endpoint 추적
- 실제 바이트 수와 datagram 수 추적
- 실제 로그 파일 누적
...
- `UDP_LAB_PORT`
  `Real UDP Lab`의 기본 bind 포트. 기본값은 `4301`

포트 충돌이 있으면 예를 들어 이렇게 실행할 수 있다.

```bash
PORT=3004 TCP_PORT=4110 UDP_PORT=4310 TCP_LAB_PORT=4210 UDP_LAB_PORT=4302 npm start
```
...
### G. Real UDP Lab 빠른 사용법

1. 상단 탭에서 `실제 UDP 실험실`로 이동한다.
2. `바인드 호스트`에 `127.0.0.1`, `포트`에 `4301`을 둔다.
3. `서버 시작`을 누른다.
4. 상태 줄에 `127.0.0.1:4301 에서 실제 UDP 서버 소켓이 바인드돼 있다.`가 보이면 실제 UDP 서버 소켓이 열린 것이다.
5. `클라이언트 바인드`를 누른다.
6. `Datagram 제어`에서 `클라이언트 소켓`을 선택한다.
7. `대상 호스트=127.0.0.1`, `대상 포트=4301`을 확인하고 `Datagram 전송`을 누른다.
8. `실시간 UDP 로그`에서 `Datagram 전송`과 `Datagram 수신`이 이어서 찍히는지 확인한다.
9. 외부 도구로 보려면 별도 터미널에서 `nc -u 127.0.0.1 4301`을 사용한다.
