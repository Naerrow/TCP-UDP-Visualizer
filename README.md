# TCP Visualizer + Real TCP Lab

Node.js `net` 소켓으로 TCP를 두 방식으로 학습하고 확인하는 프로젝트다.
...
즉, 이 프로젝트는 "개념 학습용 화면"과 "실제 TCP 실험실"을 함께 제공한다.
...
### Learning Demo의 정확성 기준

이 화면은 실제 TCP 소켓을 사용하지만, `SYN`, `SYN-ACK`, `ACK`, `FIN`은 학습용 개념 이벤트로 설명한다.

즉, 이 모드는 "앱에서 무엇이 보이고, 그 순간 TCP는 개념적으로 무엇을 하는가"를 학습시키는 시각화다.
...
- `PORT`
  웹 UI 포트. 기본값은 `3000`
- `TCP_PORT`
  `Learning Demo`에서 사용하는 학습용 내부 TCP 포트. 기본값은 `4100`
- `TCP_LAB_PORT`
...
```bash
PORT=3004 TCP_PORT=4110 TCP_LAB_PORT=4210 npm start
```

## Verify

기존 `Learning Demo`는 아래 명령으로 검증할 수 있다.
